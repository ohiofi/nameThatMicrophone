...
if (isset($_SESSION['MM_Username'])) {
  $colname_rsGame6 = $_SESSION['MM_Username'];
}
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsGame6=sprintf("SELECT game6responses.entryNumber, game6responses.questionNumber FROM game6responses WHERE userName=%s AND game6responses.tally=1 ORDER BY game6responses.questionNumber DESC",
  GetSQLValueString($colname_rsGame6, "text")); 
$rsGame6 = mysql_query($query_rsGame6, $ohiofi) or die(mysql_error());
$row_rsGame6 = mysql_fetch_assoc($rsGame6);
$totalRows_rsGame6 = mysql_num_rows($rsGame6);




mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScore3=sprintf("SELECT game6responses.entryNumber, game6responses.questionNumber FROM game6responses WHERE userName=%s",
  GetSQLValueString($colname_rsGame6, "text")); 
...
/* ------------------------------------- rsGame6_check is the fail safe -----------------------------------------*/

mysql_select_db($database_ohiofi, $ohiofi);
$query_rsGame6_check=sprintf("SELECT game6responses.entryNumber, game6responses.questionNumber FROM game6responses WHERE userName=%s AND game6responses.questionNumber=%s",
  GetSQLValueString($colname_rsGame6, "text"), GetSQLValueString($_POST['questionNumber'], "int")); 
$rsGame6_check = mysql_query($query_rsGame6_check, $ohiofi) or die(mysql_error());
$row_rsGame6_check = mysql_fetch_assoc($rsGame6_check);
$totalRows_rsGame6_check = mysql_num_rows($rsGame6_check);


mysql_select_db($database_ohiofi, $ohiofi);
$query_rsUser=sprintf("SELECT users.userID, users.game6 FROM users WHERE userName=%s ",
  GetSQLValueString($colname_rsGame6, "text")); 
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game6 FROM users ORDER BY game6 DESC";
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game6 FROM users WHERE game6 > 0 ORDER BY game6 DESC";
$query_limit_rsScoreboard = sprintf("%s LIMIT %d, %d", $query_rsScoreboard, $startRow_rsScoreboard, $maxRows_rsScoreboard);
$rsScoreboard = mysql_query($query_limit_rsScoreboard, $ohiofi) or die(mysql_error());
$row_rsScoreboard = mysql_fetch_assoc($rsScoreboard);
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	if ($totalRows_rsGame6_check == 0) {
		  $updateSQL = sprintf("UPDATE users SET game6=%s, game6total=%s WHERE userID=%s",
							   GetSQLValueString($_POST['points'], "int"),
							   GetSQLValueString($_POST['game6total'], "int"),
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
	if ($totalRows_rsGame6_check == 0) {
		  $updateSQL = sprintf("UPDATE users SET game6total=%s WHERE userID=%s",
							   GetSQLValueString($_POST['game6total'], "int"),
							   GetSQLValueString($_POST['userNumber'], "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	if ($totalRows_rsGame6_check == 0) {
		///if ($row_rsScore3['questionNumber'] == $totalRows_rsScore3){
	  		...
	  		$insertSQL = sprintf("INSERT INTO game6responses (entryNumber, userName, questionNumber, question, answer, tally) VALUES (%s, %s, %s, %s, %s, %s)",
...
<div id="footer">
            ...
            <div id="tryAgain" class="popup">
            	<h2>Incorrect</h2>
       		  <form id="form2" name="form2" action="<?php echo $editFormAction; ?>" method="POST"><input name="entryNumber" type="hidden" value="" /><input name="userName" type="hidden" value="" /><input type="hidden" name="question2" id="question2" value=""><input type="hidden" name="response2" id="response2" value=""><input name="tally" type="hidden" value="0" /><input name="questionNumber" type="hidden" value="<?php echo $currentQuestion; ?>" /><input name="userNumber" type="hidden" value="<?=$row_rsUser["userID"] ?>" /><input name="game6total" type="hidden" value="<?php echo $totalRows_rsScore3; ?>" /><input class="submit button popupbutton" type="submit" name="Submit" id="Submit" value="Continue">
                ...
                <? if ($currentQuestion % 1 == 0) { ?>
                	...
                	<h2>Your current score is<br /><? echo $row_rsUser["game6"] ?> <? if ($row_rsUser["game6"] != 1)
                                echo " pts";
                            else
                                echo " pt";
                            ?></h2>
                      ...
                      <table border="0" STYLE="margin:15px;">
                        ...
                        do { ?>
                          <tr>
                            ...
                            <td><?php echo " "; ?></td>
                            <td><?php echo " ",$row_rsScoreboard['game6'];
                            if ($row_rsScoreboard['game6'] != 1)
                                echo " pts";
                            else
                                echo " pt";
                            ?>
...
function newQuestion() {
	...
	while ((randomNumber <=3)||(randomNumber>=7&&randomNumber<=9)||(randomNumber>=19&&randomNumber<=30)||(randomNumber>=34&&randomNumber<=36)||(randomNumber>=40&&randomNumber<=42)||(randomNumber>=49&&randomNumber<=75)||(randomNumber>=103)) {
		var randomNumber=Math.floor(Math.random()*<?php echo ($totalNumberOfSlides*3) ?>+1);
	}
...
mysql_free_result($rsGame6);

mysql_free_result($rsGame6_check);
