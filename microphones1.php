...
if (isset($_SESSION['MM_Username'])) {
  $colname_rsGame5 = $_SESSION['MM_Username'];
}
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsGame5=sprintf("SELECT game5responses.entryNumber, game5responses.questionNumber FROM game5responses WHERE userName=%s AND game5responses.tally=1 ORDER BY game5responses.questionNumber DESC",
  GetSQLValueString($colname_rsGame5, "text")); 
$rsGame5 = mysql_query($query_rsGame5, $ohiofi) or die(mysql_error());
$row_rsGame5 = mysql_fetch_assoc($rsGame5);
$totalRows_rsGame5 = mysql_num_rows($rsGame5);




mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScore3=sprintf("SELECT game5responses.entryNumber, game5responses.questionNumber FROM game5responses WHERE userName=%s",
  GetSQLValueString($colname_rsGame5, "text")); 
...
/* ------------------------------------- rsGame5_check is the fail safe -----------------------------------------*/

mysql_select_db($database_ohiofi, $ohiofi);
$query_rsGame5_check=sprintf("SELECT game5responses.entryNumber, game5responses.questionNumber FROM game5responses WHERE userName=%s AND game5responses.questionNumber=%s",
  GetSQLValueString($colname_rsGame5, "text"), GetSQLValueString($_POST['questionNumber'], "int")); 
$rsGame5_check = mysql_query($query_rsGame5_check, $ohiofi) or die(mysql_error());
$row_rsGame5_check = mysql_fetch_assoc($rsGame5_check);
$totalRows_rsGame5_check = mysql_num_rows($rsGame5_check);


mysql_select_db($database_ohiofi, $ohiofi);
$query_rsUser=sprintf("SELECT users.userID, users.game5 FROM users WHERE userName=%s ",
  GetSQLValueString($colname_rsGame5, "text")); 
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game5 FROM users ORDER BY game5 DESC";
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game5 FROM users WHERE game5 > 0 ORDER BY game5 DESC";
$query_limit_rsScoreboard = sprintf("%s LIMIT %d, %d", $query_rsScoreboard, $startRow_rsScoreboard, $maxRows_rsScoreboard);
$rsScoreboard = mysql_query($query_limit_rsScoreboard, $ohiofi) or die(mysql_error());
$row_rsScoreboard = mysql_fetch_assoc($rsScoreboard);
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	if ($totalRows_rsGame5_check == 0) {
		  $updateSQL = sprintf("UPDATE users SET game5=%s, game5total=%s WHERE userID=%s",
							   GetSQLValueString($_POST['points'], "int"),
							   GetSQLValueString($_POST['game5total'], "int"),
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
	if ($totalRows_rsGame5_check == 0) {
		  $updateSQL = sprintf("UPDATE users SET game5total=%s WHERE userID=%s",
							   GetSQLValueString($_POST['game5total'], "int"),
							   GetSQLValueString($_POST['userNumber'], "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	if ($totalRows_rsGame5_check == 0) {
		///if ($row_rsScore3['questionNumber'] == $totalRows_rsScore3){
	  		...
	  		$insertSQL = sprintf("INSERT INTO game5responses (entryNumber, userName, questionNumber, question, answer, tally) VALUES (%s, %s, %s, %s, %s, %s)",
...
<div id="footer">
            ...
            <div id="tryAgain" class="popup">
            	<h2>Incorrect</h2>
       		  <form id="form2" name="form2" action="<?php echo $editFormAction; ?>" method="POST"><input name="entryNumber" type="hidden" value="" /><input name="userName" type="hidden" value="" /><input type="hidden" name="question2" id="question2" value=""><input type="hidden" name="response2" id="response2" value=""><input name="tally" type="hidden" value="0" /><input name="questionNumber" type="hidden" value="<?php echo $currentQuestion; ?>" /><input name="userNumber" type="hidden" value="<?=$row_rsUser["userID"] ?>" /><input name="game5total" type="hidden" value="<?php echo $totalRows_rsScore3; ?>" /><input class="submit button popupbutton" type="submit" name="Submit" id="Submit" value="Continue">
                ...
                <? if ($currentQuestion % 1 == 0) { ?>
                	...
                	<h2>Your current score is<br /><? echo $row_rsUser["game5"] ?> <? if ($row_rsUser["game5"] != 1)
                                echo " pts";
                            else
                                echo " pt";
                            ?></h2>
                      ...
                      <table border="0" STYLE="margin:15px;">
                        ...
                        do { ?>
                          <tr>
                            ...
                            <td><?php echo " "; ?></td>
                            <td><?php echo " ",$row_rsScoreboard['game5'];
                            if ($row_rsScoreboard['game5'] != 1)
                                echo " pts";
                            else
                                echo " pt";
                            ?>
...
function newQuestion() {
	...
	while ((randomNumber >=4&&randomNumber<=6)||(randomNumber>=31&&randomNumber<=33)||(randomNumber>=37&&randomNumber<=39)||(randomNumber>=10 && randomNumber<=18)||(randomNumber>=43 && randomNumber<=48)||(randomNumber>=76 && randomNumber<=102)) {
		var randomNumber=Math.floor(Math.random()*<?php echo ($totalNumberOfSlides*3) ?>+1);
	}
...
mysql_free_result($rsGame5);

mysql_free_result($rsGame5_check);
